...
our @RRDDefaultArgs = ('-w', '600','-h','150');
...
sub validate_args
{
	...
	if ($Args->{'path_rrd'} && ($Args->{'path_rrd'} =~ m#/#))
	{
		delete ($Args->{'path_rrd'});
	}
...
sub main
{
	#we commant this we dont need to load the confgi from the file
	#read_config ();
	
	
	validate_args ();
	my $value= param ('path_rrd');
	push (@DataDirs, $value);
